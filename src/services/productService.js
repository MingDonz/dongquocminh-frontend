...
// 8. Lấy sản phẩm xem nhiều
export const getByViewProducts = async (limit = 4) => {
  const res = await axiosInstance.get("/products/viewed", {
    params: { limit }
  });
  return res.data;
};
...
// 9. Lấy sản phẩm hot
export const getHotProducts = async (limit = 4) => {
  const res = await axiosInstance.get("/products", {
